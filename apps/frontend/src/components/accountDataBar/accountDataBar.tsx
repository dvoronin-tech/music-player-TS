...
const TextSpan = styled.span<ITextSpan>`
	...
	color: ${({ $color, theme }) =>
		$color === 'content' ? theme.text : theme.textDisable};
`;
...
const LogoutBtn = styled.button<{ $isPressed: boolean }>`
	width: 100%;
	height: 40px;
	border: ${({ theme, $isPressed }) =>
		$isPressed ? '1px solid transparent' : '1px solid' + theme.text};
	border-radius: 10px;
	font-size: 1.8rem;
	font-weight: 600;
	font-family: 'Rubik';
	...
	&::after {
		...
		height: 38px;
		border-radius: 10px;
		transition: ${({ $isPressed }) => ($isPressed ? '1s' : '0.4s')} linear
			all;
...
const AccountDataBar: FC = () => {
	...
	const handleChangeFileInput = (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (selectedFile) {
			...
			fileReader.onloadend = () =>
				void setPreview(fileReader.result as string);
		...
		if (changePhoto) {
			return (
				<form>
					...
					/>
					{!preview && (
						<label htmlFor="user_img">Выберите файл</label>
					)}
					{preview && (
						<FileImgWrapper>
					...
					<AccountInfo>
						...
						<div style={{ marginBottom: '20px' }}>
							...
							<TextSpan
								...
								$color="disable"
							>
								{regDate.getDate()} {renderMonth(regDate)}
							</TextSpan>
						</div>
						<div style={{ marginBottom: '20px' }}>
							<TextSpan
	...
	return (
		<AccountBar $isShow={showUserData}>
			<FlexRow>
				<TextSpan $fontSize="big" $fontWeight="bold" $color="content">
					Аккаунт
				</TextSpan>
				<BackBtn onClick={() => dispatch(toggleShowUserData(false))}>
					<Cross />
				</BackBtn>
			</FlexRow>
			{renderData()}
